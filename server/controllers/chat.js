...
exports.crear_dar_conversacion = function(req, res, next){
	...
	if (req.body.destinatario !== "general") {
		async.waterfall([
			function(callback){
				Chat.findOne({$or : [{$and : [{remitente : req.session.passport.user._id.toString()}, {destinatario : req.body.destinatario}]},
				 {$and : [{destinatario : req.session.passport.user._id.toString()}, {remitente : req.body.destinatario}]}]})
				.populate('remitente')
